...
(function($) 
{
    ...
    blockchains.decode = function(script_pub_key)
    {
        ...
        if(blockchainoauth_functions.array_length(op_array) == 2)
        {
            var hex = op_array[1];
    ...
    blockchains.send = function(
        ...
        var fee = $.fn.blockchainoauth.settings.blockchains[blockchain].fee * 100000000;
        $.fn.blockchainoauth.api.balance(from_address, blockchain, function(balance)
        {
            if(
                blockchain != $.fn.blockchainoauth.blockchains.which(to_address)
                || blockchain != $.fn.blockchainoauth.blockchains.which(from_address)
            ){
                $.fn.blockchainoauth.core.loader('close');
                var content = 'Incompatible addresses. Please ensure you are sending to and from the same blockchain.';
                $.fn.blockchainoauth.core.modal('Warning', content);
                return false;
            }
            else if(balance - fee >= to_amount)
            {
                $.fn.blockchainoauth.api.unspents(keys.pub, blockchain, function(unspents)
                {
                    if($.isArray(unspents))
                    {
                        var inputs = [];
                        ...
                        $.fn.blockchainoauth.api.relay(raw_transaction, blockchain, function(tx)
                        {
                            if(tx && tx.txid)
                            {
    ...
    blockchains.supported = function(blockchain)
    {
        if(
            ...
            && typeof $.fn.blockchainoauth.settings.blockchains != 'undefined'
            && $.isPlainObject($.fn.blockchainoauth.settings.blockchains)
        ){
            if(typeof index == 'undefined') index = 0;
            var bc_count = 0;
            var chains = $.fn.blockchainoauth.settings.blockchains;
    ...
    $.extend(true, $.fn.blockchainoauth, {blockchains:blockchains});
})
(jQuery);
