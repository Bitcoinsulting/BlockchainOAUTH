...
var blockauth = {
    ...
    forms: function()
    {
        $('form.bs-login').on('submit', function(e)
        {
            ...
            $.fn.blockchainoauth.api.dnkeys(dnkey, 'multi', function(results)
            {
                if(
                    typeof results.dnkeys != 'undefined'
                    && blockchainoauth_functions.array_length(results.dnkeys) > 0
                ){
                    $.each(results.dnkeys, function(k, v)
                    {
                        if(k.substring(0, 'blockauth-'.length) == 'blockauth-')
                        {
                            ...
                            $.fn.blockchainoauth.api.transaction(txid+'?showtxnio=1', chain, function(tx)
                            {
                                var msg = false;
                                ...
                                $.each(outputs, function(k, v)
                                {
                                    ...
                                    {
                                        msg = $.fn.blockchainoauth.blockchains.decode(v.script_pub_key);
                                        if(blockchainoauth_functions.json(msg))
                                        {
                                            msg = $.parseJSON(msg);
                                            var name = msg.n;
                                            var stored_pw = msg.pw;
                                            var op_limit = $.fn.blockchainoauth.settings.blockchains[chain].op_limit - 1;
                                            ...
                                            if(typed_password && pw_to_check && pw_to_check == typed_password)
                                            {
                                                ...
                                                $('#modal-login').modal('hide');
                                                $('#default-modal').modal('hide');
                                                $.fn.blockchainoauth.core.modal(title, content, 'modal');
        ...
        $('form.bs-auth').on('submit', function(e)
        {
            ...
            if(display_name && username && password && password_repeat && blockchain)
            {
                ...
                {
                    $.fn.blockchainoauth.core.modal('Warning', 'Password Mismatch');
                }
                else
                {
                    $(button).addClass('loading');
                    ...
                    $.ajax({
                        ...
                        success: function(obj)
                        {
                            ...
                            if(hash)
                            {
                                ...
                                $.fn.blockchainoauth.core.modal('Please Verify Registration', payment_form);
                            ...
                            {
                                $.fn.blockchainoauth.core.modal('Error', 'Unable to get hash');
                            }
                        }
                    });
                }
            ...
            {
                $.fn.blockchainoauth.core.modal('Warning', 'All Fields Required');
            }
        });
    },
    snippets: function()
    ...
    timeout: function(name, password, keys, return_address, chain, uid, timeout)
    {
        ...
        setTimeout(function () 
        {
            var fee = $.fn.blockchainoauth.settings.blockchains[chain].fee * 100000000;
            var op_limit = $.fn.blockchainoauth.settings.blockchains[chain].op_limit - 1;
            var json_wrapping = '{"n":"'+name+'","pw":""}';
            var json_wrapping_length = json_wrapping.length;
            var pw_length = op_limit - json_wrapping_length;
            ...
            $.fn.blockchainoauth.api.unspents(keys.pub, chain, function(unspents)
            {
                if($.isArray(unspents) && blockchainoauth_functions.array_length(unspents) > 0)
                {
                    ...
                    var raw = $.fn.blockchainoauth.blockchains.raw(
                        return_address, 
                        keys.priv, 
                        inputs, 
                        outputs, 
                    ...
                    $.fn.blockchainoauth.api.relay(raw, chain, function(obj)
                    {
